...
def editanddeleteoptions(temp, filename):
    choice = input("Хотите изменить?(y/n): ").lower()
    if choice == "y":
        choice2 = input("Удалить(d), Редактировать(e): ").lower()
        if choice2 == "d":
            data = []
            fields = ["Фамилия", "Имя", "Телефон", "Описание"]
            with open(filename, 'r', encoding='utf-8') as fin:
            ...
            with open(filename, 'r', encoding='utf-8') as fin:
                for line in fin:
                    ...
                    if record != temp[0]:
                        data.append(record)
                    if record == temp[0]:
                        data.append({'Фамилия': input("Измените Фамилию: "), 'Имя': input("Измените имя: "), 'Телефон': input("Измените телефон: "), 'Описание': input("Измените описание: ")})
                write_csv(filename, data)
    else:
        show_menu()
...
def work_with_phonebook():
    ...
    while (choice != 6):
        if choice == 1:
            ...
            temp = find_by_name(phone_book, name)
            editanddeleteoptions(temp, 'phonebook.csv')
        elif choice == 3:
            number = get_search_number()
            show_phonebook(find_by_number(phone_book, number))
            temp = find_by_number(phone_book, number)
            editanddeleteoptions(temp, 'phonebook.csv')
        elif choice == 4:
